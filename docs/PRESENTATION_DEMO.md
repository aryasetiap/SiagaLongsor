...
To verify the live demo, sign in as the isolated Demo Owner, confirm the presentation banner, one enabled Presentation Simulator device, incoming history on Overview, and the progression `SAFE` → `WATCH` → `WARNING` → `DANGER` → `UNKNOWN` → `SAFE`. `UNKNOWN` is generated by the deliberate required-sensor gap. Never use a physical ESP32 credential with this environment.
